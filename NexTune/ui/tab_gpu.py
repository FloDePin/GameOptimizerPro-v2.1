...
class GpuTunerTab(tk.Frame):
    ...
    def _build_autotune(self, p):
        ...
        # Log
        SecHdr(p, "Tuner Log").pack(fill="x", padx=12, pady=(6, 0))
        self.log = LogBox(p)
        self.log.pack(fill="both", expand=True, padx=12, pady=(2, 8))
    ...
    def _apply_mode_to_config(self, mode_id: str):
        """When mode changes, adjust visible defaults slightly."""
        try:
            from core.gpu_defaults import get_defaults
            gpu_name = self.monitor.read().name
            d = get_defaults(gpu_name)
            ...
            if mode_id == "oc_only":
                # OC only: can push core harder, power stays 100
                self.v_core_max.set(d.core_max_mhz)
                self.v_pwr_min.set(100)   # Not used in OC only
            elif mode_id == "uv_only":
                # UV only: core stays 0, can go lower on power
    ...
    def _start_tune(self):
        ...
        mode_str = {"oc_only": "Overclock Only", "uv_only": "Undervolt Only",
                    "oc_uv": "OC + Undervolt"}.get(self.v_mode.get(), "OC + UV")
