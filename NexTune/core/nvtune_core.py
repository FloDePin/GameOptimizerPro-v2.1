...
class AfterburnerController:
    ...
    def write_and_apply(self, slot: int, profile: TuneProfile) -> tuple[bool, str]:
        ...
        # V/F curve lock point (if set)
        if profile.lock_voltage_mv > 0 and profile.lock_freq_mhz > 0:
            # Afterburner VF curve: write a flat curve at lock_voltage_mv
            # The curve format is: 1024 frequency points (mV steps from 0 to 1200)
            # We write a simple flat-line at lock_freq_mhz above lock_voltage_mv
            existing["VFCurveEnabled"] = "1"
            existing["VFCurveLockVoltage"] = str(profile.lock_voltage_mv)
            existing["VFCurveLockFrequency"] = str(profile.lock_freq_mhz)

        try:
            os.makedirs(self.profile_dir, exist_ok=True)
            with open(path, "w", encoding="utf-8") as f:
                f.write(f"; NVTuner v2 profile: {profile.name}\n")
